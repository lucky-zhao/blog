...
2. 某个线程通过CAS方式操作某个变量不成功，长时间自旋，则会造成CPU空转，浪费性能。 
...
该方法是获取共享资源的顶层方法，定义了获取资源成功/失败后的逻辑，里面调用了`tryAcquire`方法，`tryAcquire`方法是用来尝试获取资源，但是在AQS里并没有实现，有不同的实现类根据自己情况去实现。
